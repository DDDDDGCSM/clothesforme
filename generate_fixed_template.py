#!/usr/bin/env python3
"""
生成修复后的UI模板 - 修复头像、移除历史交换、改为上下滑动、修复分类
"""
import json
...
<body>
    ...
    <script>
        ...
        function getAvatarUrl(user) {{
            // 如果头像URL存在且有效，使用它；否则生成SVG占位符
            if (user.avatar && user.avatar.startsWith('http')) {{
                return user.avatar;
            }}
            // 生成SVG占位符
            const initial = user.name.charAt(0).toUpperCase();
            const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="55" height="55"><rect width="55" height="55" fill="#d4af37"/><text x="50%" y="50%" fill="#1a1a1a" text-anchor="middle" dominant-baseline="central" font-size="26" font-weight="bold">${{initial}}</text></svg>`;
                        ...
                        <div class="clothes-info">
                            ...
                            <div class="user-info">
                                <div class="user-avatar">
                                    <img src="${{avatarUrl}}" alt="${{item.user.name}}" onerror="this.onerror=null; this.src='${{avatarFallback}}'; this.style.display='block';">
                                </div>
